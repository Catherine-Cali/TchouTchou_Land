...
class Type_de_train{
	...
	static Loco2w			= new Type_de_train('locomotive et 2 wagons');

	static Loco3w			= new Type_de_train('locomotive et 3 wagons');

	static Loco5w			= new Type_de_train('locomotive et 5 wagons');
...
const IMAGE_VACHE2 = new Image();
IMAGE_VACHE2.src = 'images/vache2.png';
		...
		switch(train.direction){
			case "droite" :
				if(train.x+1 < LARGEUR_PLATEAU){
					...
					if(next_case === Type_de_case.Rail_droite_vers_haut){
						train.direction = "haut";
					}
					if(next_case === Type_de_case.Rail_droite_vers_bas){
						train.direction = "bas";
					}
					if( (next_case === Type_de_case.Rail_horizontal) || (next_case === Type_de_case.Rail_droite_vers_haut) || (next_case === Type_de_case.Rail_droite_vers_bas)){
			...
			case "gauche" :
				if(train.x-1 >= 0){
					next_case = plateau.cases[train.x-1][train.y];
					if(next_case === Type_de_case.Rail_haut_vers_droite){
						train.direction = "bas";
					}
					if(next_case === Type_de_case.Rail_bas_vers_droite){
						train.direction = "haut";
					}
					if( (next_case === Type_de_case.Rail_horizontal) || (next_case === Type_de_case.Rail_haut_vers_droite) || (next_case === Type_de_case.Rail_bas_vers_droite)){
						avancer(contexte,plateau,train,train.x-1,train.y);
					}
					else{
						détruire(contexte,plateau,train);
			...
			case "haut" :
				if(train.y-1 >= 0){
					...
					if( (next_case === Type_de_case.Rail_vertical) || (next_case === Type_de_case.Rail_haut_vers_droite) || (next_case === Type_de_case.Rail_droite_vers_bas)){
			...
			case "bas" :
				if(train.y+1 < HAUTEUR_PLATEAU){
					...
					if( (next_case === Type_de_case.Rail_vertical) || (next_case === Type_de_case.Rail_droite_vers_haut) || (next_case === Type_de_case.Rail_bas_vers_droite)){
						avancer(contexte,plateau,train,train.x,train.y+1); 
					}
					else{
						détruire(contexte,plateau,train);
...
function maj_train(contexte,plateau){
	if(en_pause === 1){
		...
		for(let i = 0; i < nb_trains;i++){
			...
			nb_trains = nb_trains - destruction;
			for(let i = 0; i < trains_supp.length; i++){
				trains.splice(trains_supp[i],1);
			}
		}
	}
}
...
function dessiner_monstre(contexte,x,y){ //Dessine le mosntre en x,y
	contexte.drawImage(IMAGE_MONSTRE1, x * LARGEUR_CASE, y * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	contexte.drawImage(IMAGE_MONSTRE2, x * LARGEUR_CASE, y+1 * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	contexte.drawImage(IMAGE_MONSTRE3, x+1 * LARGEUR_CASE, y * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	contexte.drawImage(IMAGE_MONSTRE4, x+1 * LARGEUR_CASE, y+1 * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
...
function vache(contexte,plateau,x,y){
	contexte.drawImage(IMAGE_VACHE1, x * LARGEUR_CASE, y * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	sleep(3000).then( () => dessine_case(contexte, plateau,x,y));
	console.log("attente finit");
	contexte.drawImage(IMAGE_VACHE2, x * LARGEUR_CASE, y * HAUTEUR_CASE, LARGEUR_CASE, HAUTEUR_CASE);
	//sleep(500).then( () => dessine_case(contexte, plateau,x,y));
}
...
function cree_plateau_initial(plateau){
	...
	// Gare
	plateau.cases[29][8] = Type_de_case.gare1;
	plateau.cases[29][9] = Type_de_case.gare2;
	plateau.cases[29][10] = Type_de_case.gare3;
	plateau.cases[30][8] = Type_de_case.gare4;
	plateau.cases[30][9] = Type_de_case.gare5;
	plateau.cases[30][10] = Type_de_case.gare6;
	plateau.cases[31][8] = Type_de_case.gare7;
	plateau.cases[31][9] = Type_de_case.gare8;
	plateau.cases[31][10] = Type_de_case.gare9;
	plateau.cases[32][8] = Type_de_case.gare10;
	plateau.cases[32][9] = Type_de_case.gare11;
	plateau.cases[32][10] = Type_de_case.gare12;
...
function tchou(){
	...
	dessiner_monstre(contexte,0,0);

	setInterval(maj_train,500,contexte,plateau);
	setInterval(clignotement_aiguillage,500,boutons[8]);
	setInterval(vache,3000,contexte,plateau,5,4); 

	// a faire verifier à arthur 
	function playAudioAtRandomIntervals(audioNom, minDelay, maxDelay, times) {
		if (times <= 0) {
			return;
		}
		...
		setTimeout(() => {
			audio.play();
			playAudioAtRandomIntervals(audioNom, minDelay, maxDelay, times - 1); // Appel récursif pour jouer le son à nouveau
		}, randomDelay);
	}
	
	// Utilisation :
	const minDelay = 10; // Minimum 1 seconde
	const maxDelay = 40000; // Maximum 4 secondes
	const times = 10; // Nombre de fois à jouer le son
	
	if(nb_trains > 0){
		console.log(nb_trains)
		playAudioAtRandomIntervals("son/trainvap.mp3", minDelay, maxDelay, times);
	}
